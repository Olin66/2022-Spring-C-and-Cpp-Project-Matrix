...
namespace mat {
    namespace ex {
        ...
        public:
            ...
            const char *what() override {
                switch (choice) {
                    ...
                    case 2:
                        return (new std::string("The matrix exception occurs when " + message + " with invalid row " +
                                                std::to_string(i1) + " and " + std::to_string(i2)))->c_str();
                    case 3:
                        return (new std::string(
                                "The maxtrix exception occurs when " + message + "with invalid column " +
                                std::to_string(i1) + " and " + std::to_string(i2)))->c_str();
                    default:
                        return "";
                }
        ...
        public:
            const char *what() override {
                return (new std::string(
                        "The matrix exception occurs when creating SparseMatrix with duplicated triples!"))->c_str();
            }
        };
        ...
        public:
            ...
            const char *what() override {
                return (new std::string(
                        "The matrix exception occurs when creating SparseMatrix with invalid triple index = " +
                        std::to_string(i1) + " and " + std::to_string(i2) + "!"))->c_str();
            }
        };
        ...
        public:
            ...
            template<class T>
            MismatchedSizeException(const Matrix<T> &l_mat, const Matrix<T> &r_mat, std::string message): l_row(
                    l_mat.getRow()), l_col(l_mat.getCol()), r_row(r_mat.getRow()), r_col(r_mat.getCol()) {};

            template<class T>
            MismatchedSizeException(const BasicMatrix<T> &l_mat, const BasicMatrix<T> &r_mat, std::string message)
                    :l_row(l_mat.getRow()), l_col(l_mat.getCol()), r_row(r_mat.getRow()), r_col(r_mat.getCol()) {};
        ...
        public:
            ...
            template<class T>
            NotSquareException(const BasicMatrix<T> &mat, std::string message):row(mat.getRow()), col(mat.getCol()),
                                                                               operation(message) {};

            template<class T>
            NotSquareException(const SparseMatrix<T> &mat, std::string message):row(mat.getRow()), col(mat.getCol()),
                                                                                operation(message) {};

            const char *what() override {
                return (new std::string(
                        "The matrix exception occurs when " + operation + "\nThe matrix is not square!"))->c_str();
            }
        };
        ...
        public:
            ...
            template<class T>
            NoInverseException(const BasicMatrix<T> &mat, std::string message):row(mat.getRow()), col(mat.getCol()),
                                                                               operation(message) {};

            template<class T>
            NoInverseException(const SparseMatrix<T> &mat, std::string message):row(mat.getRow()), col(mat.getCol()),
                                                                                operation(message) {};

            const char *what() override {
                return (new std::string(
                        "The matrix exception occurs when doing " + operation + "\nThe matrix does not have inverse!"
                ))->c_str();
            }
        };
    }
