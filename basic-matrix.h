...
template <class T>
void BasicMatrix<T>::uptrangle(){
    for (int i = 0; i < this->col; i++)
    {
        int j;
        for (j = i; j < this->row; j++)
        {
            if (fabs(this->getByIndex(j,i))>0.000001)
            {
                break;
            }
        ...
        if (j<this->col)
        {
            ...
            {
                T temp=this->getByIndex(i,k);
                this->setByIndex(i,k,this->getByIndex(j,k));
                this->setByIndex(j,k,temp);
            }
            T times;
            for (int k = i+1; k < this->row; k++)
            {
                times=-this->getByIndex(k,i)/this->getByIndex(i,i);
                for (j = i; j < this->col; j++)
                {
                    this->setByIndex(k,j,this->getByIndex(k,j)+times*this->getByIndex(i,j));
                }
